...
class Dualpath_block_v1(torch.nn.Module):
    def __init__(self, num_heads, feature_size, rnn_layers=2):
        super(Dualpath_block_v1, self).__init__()
...
class SpatioTemporal_block_v1(torch.nn.Module):
    def __init__(self, n_heads, num_blocks, feature_size, rnn_layers):
        super(SpatioTemporal_block_v1, self).__init__()     
    ...
    def forward(self, x, MPE):
        """
        Forward pass for V1 SpatioTemporal block using MPE.
        
        Args:
            x: Feature tensor (B, C, M, T)
            MPE: Microphone positional encoding (B, C, M)
        
        Returns:
            Processed feature tensor (B, C, M, T)
        """
        B, C, M, T=x.shape
        MPE=MPE.unsqueeze(-1)
